...
print("current accuracy value:", float(layer.metrics[0].result()))   # note this result method
...
"""
What is serialization TensorFlow?
Serialization refers to the process of converting a data object
(e.g. Python objects, Tensorflow models) into a format that allows us store 
or transmit the data and then recreate the object when needed using the 
reverse process of deserialization
"""
...
class Linear(keras.layers.Layer):
    def __init__(self, units=32, **kwargs):     # notice kwargs here 
...
config = layer.get_config()
print(config)
new_layer = Linear.from_config(config)   # from_config is a class method belonging to class Linear
...
Mask-generating layers are the `Embedding` layer configured with `mask_zero=True`,
